------------------------- MODULE pdif_abstract -------------------------

EXTENDS Naturals, FiniteSets, TLC

VARIABLES loc1, loc2, locOther, msgs, previous, phase, crash, decisionRound, receivers, someP, pclean, globalDecisionRound

vars == <<loc1, loc2, locOther, msgs, previous, phase, crash, decisionRound, receivers, someP, pclean, globalDecisionRound>>

v0 == 0 \* default decision value
V == {0, 1} \* set of values
I == 1 .. 8  \* indices corresponding to process locations
U == 1 .. 10 \* message array indices (process locations + 2 concrete processes)
u1 == 9 \* index for process 1
u2 == 10 \* index for process 2
...
Correct == {u \in IndexActive : ~halted(u) /\ ~early(u) /\ (u \in {u1, u2} \/ crash[location(u)] # {TRUE})}
...
UpdateCorrect(l, u) == 
    ...
    THEN IF l.est = 1 /\ loc1.est = l.est /\ loc2.est = l.est 
...
EnvSemMsg(m, u, v) ==
    ...
    ELSE LET l == location(u) IN
         ...
         THEN IF ~early(v) /\ estValue(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})  
              ...
              THEN IF ~early(v) /\ estValue(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN IF receivers[l,v] = {TRUE}
                        ...
                        ELSE IF receivers[l,v] = {FALSE}
                             ...
                             ELSE {<<>>, m}
                   ELSE {<<>>}          
              ELSE IF ~early(v) /\ estValue(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN {m}
                   ELSE {<<>>}
...
\* Last modified Tue Oct 17 14:48:46 CEST 2017 by stoilkov
\* Created Tue Sep 12 16:17:07 CEST 2017 by stoilkov
