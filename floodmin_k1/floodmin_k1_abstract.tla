...
U == 1 .. 6 \* array indices (process locations + 2 fixed processes)
u1 == 5 \* index for process 1
u2 == 6 \* index for process 2
...
UpdateCorrect(l, u) ==
    IF l.min = 1 /\ loc1.min = l.min /\ loc2.min = l.min
...
EnvSemMsg(m, u, v) ==
    ...
    ELSE LET l == location(u) IN
         ...
         THEN IF min(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})  
              ...
              THEN IF min(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN IF receivers[l,v] = {TRUE}
                        ...
                        ELSE IF receivers[l,v] = {FALSE}
                             ...
                             ELSE {unknown, m}
                   ELSE {unknown}          
              ELSE IF min(v) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN {m}
                   ELSE {unknown} 
