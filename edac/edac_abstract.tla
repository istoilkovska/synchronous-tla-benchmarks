----------------------- MODULE edac_abstract -----------------------

EXTENDS Naturals, FiniteSets, TLC

VARIABLES loc1, loc2, locOther, msgs, previous, phase, crash, decisionRound, receivers, someP

vars == <<loc1, loc2, locOther, msgs, previous, phase, crash, decisionRound, receivers, someP>>

v0 == 0 \* default decision value
V == {0, 1} \* set of values
I == 1 .. 12 \* indices corresponding process locations
U == 1 .. 14 \* array indices (process locations + 2 concrete processes)
...
IndexActive == {u1, u2} \cup {index(l) : l \in locOther}  
...
Correct == {u \in IndexActive : ~halted(u) /\ ~decided(u) /\ (u \in {u1, u2} \/ crash[location(u)] # {TRUE})}
...
isDecisionRound(v) ==
    IF Cardinality(valueSet(v)) = 1
    THEN /\ \A u \in DOMAIN crash : TRUE \notin crash[u] \/ FALSE \notin receivers[u, v]   
         /\ \A u \in IndexNonHalted : equal(u, v)
    ELSE \A u \in DOMAIN crash : TRUE \notin crash[u]
...
UpdateCorrect(l, u) == 
    ...
    THEN IF Cardinality(l.W) = 1 /\ loc1.W = l.W /\ loc2.W = l.W 
...
EnvSemMsg(m, u, v) ==
    ...
    ELSE LET l == location(u) IN
         ...
         THEN IF ~decided(v) /\ Cardinality(valueSet(v)) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})  
              ...
              THEN IF ~decided(v) /\ Cardinality(valueSet(v)) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN IF receivers[l,v] = {TRUE}
                        ...
                        ELSE IF receivers[l,v] = {FALSE}
                             ...
                             ELSE {<<>>, m}
                   ELSE {<<>>}          
              ELSE IF ~decided(v) /\ Cardinality(valueSet(v)) = 1 /\ (v \in {u1, u2} \/ crash[location(v)] # {TRUE})
                   THEN {m}
                   ELSE {<<>>}
...
MsgsAlgorithm == 
    ...
    /\ UNCHANGED <<loc1, loc2, locOther, previous>> 
...
\* Last modified Wed Nov 22 20:31:58 CET 2017 by ilina
\* Last modified Tue Nov 14 17:43:13 CET 2017 by stoilkov
\* Created Mon Aug 15 13:59:16 CEST 2017 by stoilkov
