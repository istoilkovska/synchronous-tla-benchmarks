...
U == 1 .. 13 \* message array indices (process locations + 1 concrete process)
...
EnvSemMsg(m, u, v) ==
    ...
    ELSE LET l == location(u) IN
         ...
         THEN IF estValue(v) = 1 /\ (v \in {u1} \/ crash[location(v)] # {TRUE})  
              ...
              THEN IF estValue(v) = 1 /\ (v \in {u1} \/ crash[location(v)] # {TRUE})
                   THEN IF receivers[l,v] = {TRUE}
                        ...
                        ELSE IF receivers[l,v] = {FALSE}
                             ...
                             ELSE {unknown, m}
                   ELSE {unknown}          
              ELSE IF estValue(v) = 1 /\ (v \in {u1} \/ crash[location(v)] # {TRUE})
                   THEN {m}
                   ELSE {unknown}
...
\* Last modified Tue Oct 17 15:10:05 CEST 2017 by stoilkov
\* Created Thu Sep 28 17:29:23 CEST 2017 by stoilkov
